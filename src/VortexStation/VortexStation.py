...
from VortexStation.CompassWidget import CompassWidget
from VortexStation.ArtificialHorizonWidget import ArtificialHorizonWidget
...
class VortexMainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        # Add compass widget to CompassFrame
        self.compass = CompassWidget(self.CompassFrame)
        self.compassLayout = QVBoxLayout(self.CompassFrame)
        ...
        self.artificialHorizonLayout = QVBoxLayout(self.artificialHorizonFrame)
        self.artificialHorizonLayout.setContentsMargins(0, 0, 0, 0) 
        self.artificialHorizonLayout.addWidget(self.artificialHorizon)
        self.artificialHorizonFrame.setLayout(self.artificialHorizonLayout)
        ...
        self.readingsLabels = {
            ...
            Readings.Pitch: self.pitchReadingLabel,
            Readings.Roll: self.rollReadingLabel
        }
        ...
        # Controller Class, Event and Mapping
        self.pyqtController = PyqtController()
        ...
        self.pyqtCompanionController = PyqtCompanionController()
    ...
    def readingsLabelUpdater(self, key, value):
        ...
        if key == Readings.Heading:
            try:
                heading = float(value)
                self.compass.setHeading(heading)
                self.lastHeading = heading
            except ValueError:
                self.compass.setHeading(self.lastHeading)
                
        # Update artificial horizon when pitch/roll changes    
        elif key == Readings.Pitch:
            try:
                pitch = float(value)
                self.artificialHorizon.setPitch(pitch)
                self.lastPitch = pitch
            except ValueError:
                self.artificialHorizon.setPitch(self.lastPitch)
                
        elif key == Readings.Roll:
            try:
                roll = float(value)
                self.artificialHorizon.setRoll(roll)
                self.lastRoll = roll
            except ValueError:
                self.artificialHorizon.setRoll(self.lastRoll)
